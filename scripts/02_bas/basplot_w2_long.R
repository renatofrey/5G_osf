...
pdf(file=paste("output/w2_long_change/bas.pdf", sep=""), height=3.7, width=10)
#par(mfrow=c(1,4))
...
for (dv in c("PRISK", "PBENE", "POLICY")) {
  ...
  tab1 <- bas_ivs[[paste("w2_long_change_", dv, sep="")]]
  ...
  cat(dv)
  print(round(tab1[,4:ncol(tab1)], 1))
  ...
  if (dv == "PRISK") {frame(); par(mfg=c(1,2)); pm <- expression(bold(paste(Delta, " Perceived risk")))}
  if (dv == "PBENE") {par(mfg=c(1,3)); pm <- expression(bold(paste(Delta, " Perceived benefit")))}
  if (dv == "POLICY") {par(mfg=c(1,4)); pm <- expression(bold(paste(Delta, " Policy-related attitudes")))}
  ...
  if (nr == 1) pd <- NA else pd <- c(50,100)
...
lab <- as.character(rev(labels))
lab2 <- paste("Delta~", lab, sep="")
...
for (j in 1:length(ty)) {
  text(x=-.5, y=ty[j], bquote(paste(Delta~.(lab[j]))), adj=0, xpd=T, cex=1.07)
}
...
legend(x=0.32, y=1.2, horiz=F, box.lty=0,
       ...
       pch = 15, pt.cex=1.5, col=c(pc[which(rev(rownames(tab1)) == "TRUST")],
                                   pc[which(rev(rownames(tab1)) == "ehs")],
                                   gray(.3)), border=1, cex=1.05)
...
xmax <- 20
par(lwd=0.75)
b2 <- barplot(p_BFs, beside=T, horiz=T, border=c(pc2,pc2,pc2), xlim=c(-8,xmax), xpd=F, las=1, main="Importance of predictors", xaxt="n", col=pc2, density=pd2, xlab=expression('Bayes Factor (log'[10]*')'), mgp=c(1.7,0,0))
axis(1, at=c(cutmin,0,5,10,15), mgp=c(0,.5,0))
...
frame()
#abline(h=1, xpd=T, lwd=0.5)
legend(x=-.08, y=1.2, col=pc, fill=pc2, density=rev(pd2), border=pc2, c(expression(paste(Delta, " Perceived risk as outcome")), expression(paste(Delta, " Perceived benefit as outcome")), expression(paste(Delta, " Policy-related attitud. as outcome"))), box.lty=0, xpd=T, cex=1.05)
