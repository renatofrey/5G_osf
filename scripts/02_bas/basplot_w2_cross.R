...
pdf(file=paste("output/w2_cross/bas.pdf", sep=""), height=4, width=10)
#par(mfrow=c(1,4))
...
for (dv in c("PRISK", "PBENE", "POLICY")) {
  ...
  tab1 <- bas_ivs[[paste("w2_cross_", dv, sep="")]]
...
frame()
legend(x=0.33, y=1.2, horiz=F, box.lty=0,
       ...
       pch = 15, pt.cex=1.5, col=c(pc[which(rev(rownames(tab1)) == "TRUST")],
                                   pc[which(rev(rownames(tab1)) == "age")],
...
axis(1, at=c(cutmin,0,25,50,75), mgp=c(0,.5,0))
