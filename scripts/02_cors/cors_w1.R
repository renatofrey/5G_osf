...
pdf(file="output/w1/cors_prisk.pdf", width=10, height=2.5)
#pdf(file="output/combined/cors_w1.pdf", width=7.5, height=4)
...
#m <- rbind(m[,1:(ncol(m)/2)], m[,(ncol(m)/2+1):ncol(m)])
...
for(iv in ivs) {
  ...
  #legend("topright", paste("SD =", round(sd(dd,na.rm=T),0)), bg="white", box.lty=0, inset=0, y.intersp=.1,x.intersp=-.8, cex=1)
  #legend("bottomleft", paste("SD =", round(sd(dd,na.rm=T),0)), box.lty=0, inset=0, y.intersp=.1,x.intersp=-.8, cex=1)
...
frame()
text(expression(bold("Risk perception")), srt=90, x=0,y=.7, pos=1, cex=1.4, xpd=T, font=2)
