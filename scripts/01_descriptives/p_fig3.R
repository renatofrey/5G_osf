...
pdf(file="output/w2_long_change/fig3.pdf", width=6, height=4)
...
cp <- function(vx, vy, curr_cond) {
  ...
  if (curr_cond == "D_control") {
    ...
    ccol <- scales::alpha("tan3", alpha=.5)
...
frame()
text(expression(bold(paste(Delta, " Perceived risk"))), srt=90, x=0.2,y=.38, pos=1, cex=1.5, xpd=T, font=3)
