# This script implements the measurement models and stores the extracted factor values (for w1, this script also implements the SEM)
...
for (sel in c("w1", "w2_cross", "w2_long_change")) {
  ...
  print(paste("Running measurement models for ", sel, ".", sep=""))
  ...
  if (sel == "w1") print(abs(round(summary(c_diffs[lower.tri(c_diffs)]), 3)))
  cormat <- c_pears
...
POLICY =~ accep + vote + regul + resea'
  ...
  pdf(paste("output/", sel, "/cfa_mms.pdf", sep=""), width=8, height=4)
  par(mfrow=c(2,4))
  ...
  pdf(paste("output/", sel, "/dists_factors.pdf", sep=""), height=3, width=9)
  ...
  library(corrplot)
  pdf(paste("output/", sel, "/cors_factors.pdf", sep=""))
  ...
  # for w1 also runsome structural equation models (SEM)
  if (sel == "w1") source("scripts/02_cfas/cfa_sem.R")
